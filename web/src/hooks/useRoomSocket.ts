...
export function useRoomSocket({ roomId, userId }: UseRoomSocketOptions) {
  ...
  const { rooms } = useSelector((state: RootState) => state.connection);
  ...
  useEffect(() => {
    ...
    const handleMessageNew = (data: SocketMessageNew) => {
      if (data.message.roomId === roomIdRef.current) {
        ...
        // Update last seen seq
        dispatch(
          updateLastSeenSeq({ roomId: roomIdRef.current, seq: data.seq })
        );
      }
    };
    ...
    const handleRoomJoined = (data: { roomId: string; messages: any[] }) => {
      if (data.roomId === roomIdRef.current) {
        ...
        dispatch(
          setMessages({ roomId: roomIdRef.current, messages: uiMessages })
        );
        ...
        if (maxSeq > 0) {
          dispatch(
            updateLastSeenSeq({ roomId: roomIdRef.current, seq: maxSeq })
          );
        }
      }
    };
    ...
    const handleUserCount = (data: { roomId: string; count: number }) => {
      if (data.roomId === roomIdRef.current) {
        dispatch(
          setUserCount({ roomId: roomIdRef.current, count: data.count })
        );
      }
    };
